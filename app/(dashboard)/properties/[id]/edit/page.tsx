...
export default function EditPropertyPage() {
    ...
    fetch(`/api/properties/${propertyId}`)
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          setServerError(data.error || 'Failed to load property data.');
        }
      })
      .catch(() => {
        setServerError('A network error occurred while loading property data.');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setServerError(err instanceof Error ? err.message : 'Failed to update property');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 text-sm animate-pulse">Loading property details...</p>
        </div>
      </div>
    );
  }
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 px-4 py-8 sm:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8">
          <h1 className="text-xl font-bold text-slate-100 mb-6">Edit Property</h1>
